    ...
    fn register_complete(&mut self, data: web_sys::PublicKeyCredential, username: String) -> FetchTask {
        ...
        let callback = self.link.callback(
            move |response: Response<Nothing>| {
                ...
                ConsoleService::log(format!("parts -> {:?}", parts).as_str());
                ConsoleService::log(format!("body -> {:?}", _body).as_str());
                AppMsg::Toast(format!("Registration Success! parts = {:?} body = {:?}", parts, _body).to_string())
            });
            ...
            move |response: Response<Json<Result<RequestChallengeResponse, Error>>>| {
                ...
                match body {
                    Json(Ok(rcr)) => {
                        ...
                        ConsoleService::log(&format!("Got challenge! -> {:?}", deb));
                        AppMsg::BeginLoginChallenge(c_options, username_copy.clone())
                    }
                    Json(Err(_)) => {
                        ConsoleService::log("Well fuck");
                        AppMsg::DoNothing
                    }
                }
            }
            ...
            AppMsg::BeginRegisterChallenge(ccr, username) => {
                if let Some(win) = web_sys::window() {
                    ...
                    spawn_local(async move {
                        match fut.await {
                            Ok(data) => {
                                ConsoleService::log(format!("yo got data herrr: {:?}", data).as_str());
                ...
                <div id="content" class="container">
                  <div class="row d-flex justify-content-center align-items-center" style="min-height: 100vh;">
                    ...
                    <div class="col-sm-6">
                        ...
                        <div class="container">
                            <div>
                                <input id="username" type="text" class="form-control" value=self.username.clone() oninput=self.link.callback(|e: InputData| AppMsg::UserNameInput(e.value)) />
